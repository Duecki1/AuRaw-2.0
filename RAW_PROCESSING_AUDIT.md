...
The Bayer path is now a staged WGSL port of darktable's RCD 2.3 implementation rather than the former RCD-inspired approximation. It retains the reference high-pass VH and P/Q discriminators, ratio-corrected green estimates, diagonal opposite-colour interpolation, and colour-difference interpolation at green sites. The blend order was checked against darktable's `interpolatef(a, b, c) = a*b + (1-a)*c` helper.

Border behavior is explicit: a PPG result is generated for the complete image and remains authoritative in the outer nine-pixel margin, while RCD is only written where its full support is available. Reference-mode output no longer includes the previous arbitrary post-demosaic chroma smoothing.

The optional frequency-domain mode analyzes Bayer chroma energy at the three period-two carrier frequencies over an apodized 13×13 window while retaining the reference luminance. Dual mode blends the RCD result with a low-detail CFA interpolation through a radius-two Gaussian-smoothed Scharr mask and the darktable-compatible threshold mapping.

### X-Trans demosaic

The former custom four-pass X-Trans approximation has been replaced by a Markesteijn-3 pipeline with three green/chroma refinement rounds followed by eight directional candidates, perceptual YUV second derivatives, 3×3 homogeneity maps, 5×5 homogeneity sums, opposite-direction quenching, and one-eighth-of-maximum candidate selection. The 17-pixel Markesteijn-3 exterior is handled by bounded per-channel CFA interpolation that restores the measured photosite exactly.

To control full-resolution memory, directional candidates are reconstructed deterministically when derivative and accumulation stages need them instead of retaining eight RGB images simultaneously. Frequency-domain chroma uses a 13×13 X-Trans carrier analysis plus five-sample median cleanup. Dual mode uses the same radius-two Gaussian/Scharr detail-mask policy as the Bayer path.

These changes close the algorithm-structure gap identified by the audit. A pinned real-RAW corpus is still required before claiming pixel-for-pixel equivalence with darktable across cameras, CPU/GPU precision modes, and image borders.
...
1. **Reference demosaic validation**
   - The staged RCD and Markesteijn-3 ports, border policies, frequency-domain chroma, and dual modes are implemented.
   - Add pinned darktable 5.6 reference renders and per-camera Bayer/X-Trans golden crops to validate numerical and visible equivalence.
   - Keep algorithm provenance and GPL compatibility clear for every redistributed build.
