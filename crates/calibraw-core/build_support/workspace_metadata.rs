...
impl WorkspaceMetadata {
    ...
    pub(crate) fn emit_cargo_contract(&self) {
        ...
        for (name, value) in [
            (
                "CALIBRAW_ANDROID_NDK_VERSION",
                self.android_ndk_version.as_str(),
            ),
            (
                "CALIBRAW_ANDROID_BUILD_TOOLS_VERSION",
                self.android_build_tools_version.as_str(),
            ),
            ("CALIBRAW_LIBRAW_REVISION", self.libraw_revision.as_str()),
            ("CALIBRAW_LENSFUN_REVISION", self.lensfun_revision.as_str()),
        ] {
            println!("cargo:rustc-env={name}={value}");
        }
        println!(
            "cargo:rustc-env=CALIBRAW_ANDROID_COMPILE_SDK={}",
            self.android_compile_sdk
        );
        println!(
            "cargo:rustc-env=CALIBRAW_ANDROID_MIN_SDK={}",
            self.android_min_sdk
        );
        println!(
            "cargo:rustc-env=CALIBRAW_ANDROID_TARGET_SDK={}",
            self.android_target_sdk
        );
        println!(
            "cargo:rustc-env=CALIBRAW_ANDROID_USE_LEGACY_PACKAGING={}",
