...
fn test_asset(name: impl Into<PathBuf>) -> LibraryAsset {
    ...
    let uri = format!("content://auraw.test/{name}");
    LibraryAsset {
...
fn android_folder_navigation_uses_normalized_relative_paths() {
    ...
    assert_eq!(
        android_library_location_label("Android/media/de.duecki.auraw/.library", "Trips/Day 1"),
        "Android/media/de.duecki.auraw/.library/Trips/Day 1"
    );
