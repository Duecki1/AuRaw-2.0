use crate::app::{AurawApp, OnboardingStep, PreviewQuality};
use eframe::egui;
...
const ONBOARDING_LAYOUT_MEASUREMENT_ID: &str = "auraw-first-run-onboarding-layout";
const ONBOARDING_FOOTER_EXTRA_HEIGHT: f32 = 30.0;

pub(crate) fn show(ctx: &egui::Context, app: &mut AurawApp) {
    ...
    egui::Modal::new(egui::Id::new(("auraw-first-run-onboarding", step))).show(ctx, |ui| {
        ui.set_width(width);
    ...
    match action {
        ...
        Some(OnboardingAction::Finish) => {
            ...
            if !app.persist_performance_settings() {
                app.ui.notice = Some(
                    "Setup is complete, but AuRaw could not save the first-run preferences."
                        .to_owned(),
                );
            }
        }
...
fn show_step_body(ui: &mut egui::Ui, app: &mut AurawApp, step: OnboardingStep) {
    show_header(ui, step);
    ui.add_space(8.0);
    match step {
...
fn show_appearance(ui: &mut egui::Ui, app: &mut AurawApp) {
...
fn show_preview(ui: &mut egui::Ui, app: &mut AurawApp) {
...
fn show_copy_paste(ui: &mut egui::Ui, app: &mut AurawApp) {
...
#[cfg(not(target_os = "android"))]
fn show_ai(ui: &mut egui::Ui, app: &mut AurawApp) {
...
impl OnboardingStep {
    const fn title(self) -> &'static str {
        match self {
            Self::Appearance => "Welcome to AuRaw",
    ...
    const fn introduction(self) -> &'static str {
        match self {
            Self::Appearance => {
                "Choose how AuRaw looks and the background shown around your photographs."
            }
            Self::Preview => {
