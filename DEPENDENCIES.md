...
Upgrade only after running desktop dynamic-runtime tests, Android arm64 builds, subject-mask output-layout tests, SAM 2.1 encoder/decoder interface tests, iterative object-mask tests, and the full image-regression suite. A stable 2.x release should replace the RC as soon as it exposes the required APIs without regressions.
